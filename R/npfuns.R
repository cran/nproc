npc.core <- function(y, score, pred.score = NULL, alpha = NULL, delta = 0.05, n.cores = 1) {
  ...
  return(list = list(cutoff = cutoff, loc = loc, sign = sig, alpha.l = obj$alpha.l,
                     alpha.u = obj$alpha.u, beta.l = obj$beta.l, beta.u = obj$beta.u))
}
...
npc.split <- function(x, y, p, kernel, alpha, delta, ind01, ind02, ind11, ind12, method, pred.score = pred.score, n.cores = 1) {
    ...
    obj = npc.core(test.y, test.score, pred.score = pred.score, alpha = alpha, delta = delta, n.cores = n.cores)

    object = list(fit = fit, pred.y = obj$pred.y, y = test.y, score = test.score, cutoff = obj$cutoff,
        sign = obj$sign, method = method, beta.l = obj$beta.l, beta.u = obj$beta.u, alpha.l = obj$alpha.l, alpha.u = obj$alpha.u)
    class(object) = "npc"
    return(object)
}
...
## find the order such that the type-I error bound is satisfied with probability at least 1-delta
find.order <- function(score0, score1 = NULL, delta = 0.05, n.cores = 1) {
    ...
    scores = score0
    #n = n0 + n1
    #nc = n + 1 ###number of different classifiers
    ...
    alpha.mat = mcmapply(f <- function(i){if(ru0[i] == 0){
      alpha.u = 1
      alpha.l = 1
    } else if(ru0[i] == n0){
      alpha.l = 0
